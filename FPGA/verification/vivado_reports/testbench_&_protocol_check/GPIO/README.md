...
| 6 | Anode one-cold + tarama | **6a:** 2000 saat vuruşu boyunca `anode` her zaman tek-`0` (one-cold) kalıyor — iki hane aynı anda yanamaz. **6b:** `anode_select` ilerliyor, yani tarama sayacı çalışıyor | GEÇTİ (2 kontrol) |
| 7 | Ekran sönük (blank) koşulları | `IDR=0` ve `IDR=1 & ODR=0` durumlarında `catode=0xFF` (bütün segmentler sönük) | GEÇTİ (2 kontrol) |
| 8 | 7-segment mesaj tabloları | **13 mesajın tamamı, her hanesiyle** taranır: `ERASE`, `SNDING`, `FINISH`, `ERROR`, `BOOT`(varsayılan), `INFRNC`, `EVET`, `HAYIR`, `SESSIZLK`, `BILINMYN`, `YZUART`(varsayılan) — `IDR`/`ODR` kombinasyonlarıyla | GEÇTİ (12 kontrol) |
| 9 | Sistem adresiyle erişim | RTL adres kod çözmeyi yalnızca **alt 4 bit** üzerinden yapar; tam sistem adresi `0x4001_0004` ile de yazma/okuma çalışmalı | GEÇTİ (2 kontrol) |
| 10 | Arka arkaya yazma | Aralıksız iki AXI yazmasından sonra son değer geçerli | GEÇTİ |
| 11 | Tanımsız adres davranışı | **11a:** tanımsız ofsete yazma `GPIO_ODR`'yi bozmuyor. **11b:** tanımsız ofsetten okumada `rdata` **bayat kalıyor** (`case`'de `default` yok) ama `rvalid` yine yükseliyor — belgelenmiş, bilinçli davranış | GEÇTİ (1 sayılan kontrol + bilgi satırları) |
| 12 | Çalışma sırasında reset | Ortada verilen reset `GPIO_ODR`'yi sıfırlıyor, AXI tekrar boşa dönüyor ve blok sonrasında yeniden yazılabiliyor | GEÇTİ (2 kontrol) |
| 13 | Animasyon deseni | `anode_select` 0 ve 1 için 8 animasyon karesinin `catode` desenleri tek tek | GEÇTİ (16 kontrol) |
| 14 | Sayaç sarması (wrap) | **14a:** `anode_select` 7'den 0'a sarıyor. **14b:** `anim_select` 7'den 0'a sarıyor | GEÇTİ (2 kontrol) |

GPIO testbench'i geçme ölçütü olarak sabit bir sayı değil `toplam_basarisiz == 0`
kullanır; bu yüzden 14 test bloğundan 48 ayrı kontrol çıkar.
