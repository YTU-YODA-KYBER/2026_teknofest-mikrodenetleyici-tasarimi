# UART_YZ — Directed Testbench Sonucu

`UART_YZ_AXI4_Lite` (YZ veri akışı UART'ı) bloğunun **yönlendirilmiş
...
Testbench: [`UART_YZ_tb.sv`](../../../../main_codes/testbench/Peripherals/UART_YZ/UART_YZ_tb.sv) ·
Proje: `scripts/project_gen/Peripherals/create_UART_YZ.tcl` ·
...
`Bind_for_peripherals/axi4lite_bind_uart_yz.svh` ile `NAME("UART_YZ")`,
`MAX_OUTSTANDING(1)`:
...
[AXI-CHK]        UART_YZ | AW=17 W=17 B=17 AR=5 R=5 | ihlal=0 uyari=0
```
...
- **UART_YZ'nin DMA yan bandı bu testbench'te bağlı değildir.**
  `dma_enable_i` / `dma_valid_o` / `dma_data_o` portları burada sürülmez. Bu
  yol YZ girdi RAM'ini besleyen asıl farktır ve başka iki yerde doğrulanır:
  UVM `uart_yz_dma_test` ([`../../../uvm/`](../../../uvm/)) ve uçtan uca
  sistem testi ([`../AI_Accelerator/`](../AI_Accelerator/)).
- **Testbench UART_GU'nunkiyle üç satır dışında aynıdır** (bind dosyası, modül
  adı, DUT tipi). İki blok aynı RTL ailesinden türediği için bu bilinçlidir;
  ayrışan tek davranış DMA yan bandıdır.
