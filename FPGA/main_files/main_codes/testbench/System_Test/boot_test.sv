...
//
//  HAZIRLIK:
//    1) main_boot.c'yi SIM icin derle:  make sim_boot   (-DFLASHER_NO_ERASE)
//    2) makefile_outputs/sim_boot.hex'i sim dizinine 'sim_boot.hex' olarak koy
//       (boot_test.tcl bunu "Memory File" olarak zaten ekliyor).
//    3) mem_Q512.vmf'i sim dizinine KOYMA! Flash 0xFF baslamali ki flasher
//       PP ile temiz yazabilsin (sim'de erase atlaniyor).
// =====================================================================
`include "axi4lite_bind.svh"      // <-- module'den ONCE, compilation-unit scope
...
module boot_test;
    ...
    initial begin
        ...
        wait (GPIO_ODR == 32'h0003);
        $display("[TB] Flasher PASS");
        repeat (20) @(posedge clk_i);   // <-- bekleyen AXI islemleri bossalsin
    ...
    if (axi_chk_pkg::final_report() != 0) $fatal(1, "AXI protokol ihlali");
    $finish;
    end
...
always @(posedge clk_i)
    if (dut.gpio_inst.bvalid)
        $display("[GPIO-B] t=%0t bvalid=1 bready=%0b awvalid=%0b wvalid=%0b awready=%0b",
                 $time, dut.gpio_inst.bready, dut.gpio_inst.awvalid,
                 dut.gpio_inst.wvalid, dut.gpio_inst.awready);
