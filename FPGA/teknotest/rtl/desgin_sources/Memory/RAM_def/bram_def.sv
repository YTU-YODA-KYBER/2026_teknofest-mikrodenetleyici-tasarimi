module bram #(
    parameter DATA_WIDTH = 32,
    parameter ADDR_WIDTH = 10 // 2^10 = 1024 derinlik, 1024x32 bit = 4KB BRAM
)(
    input  logic                  clk,

    // Yazma Portu (Write Port)
    input  logic                  we,      // Write Enable
    input  logic [(DATA_WIDTH/8)-1:0] be,
    input  logic [ADDR_WIDTH-1:0] waddr,   // Write Address
    input  logic [DATA_WIDTH-1:0] wdata,   // Write Data

    // Okuma Portu (Read Port)
    input  logic [ADDR_WIDTH-1:0] raddr,   // Read Address
    output logic [DATA_WIDTH-1:0] rdata    // Read Data
);

    // Bellek dizisinin tanımlanması
    logic [DATA_WIDTH-1:0] ram [0:(2**ADDR_WIDTH)-1];

    // FPGA'da BRAM konfigurasyon sonrasi 0'dir. Simulasyonda X kalirsa,
    // CV32E40P'nin uygulamanin son komutundan sonra yaptigi spekulatif
    // getirmeler AXI okuma kanalina X tasir (protokol checker XCK_07).
    initial begin
        for (int i = 0; i < (2**ADDR_WIDTH); i++) ram[i] = '0;
    end

    // Yazma İşlemi
    always_ff @(posedge clk) begin
        if (we) begin
            // Sadece be=1 olan byte'ları yaz
            if (be[0]) ram[waddr][ 7: 0] <= wdata[ 7: 0];
            if (be[1]) ram[waddr][15: 8] <= wdata[15: 8];
            if (be[2]) ram[waddr][23:16] <= wdata[23:16];
            if (be[3]) ram[waddr][31:24] <= wdata[31:24];
        end
    end

    // Okuma İşlemi
    always_ff @(posedge clk) begin
        rdata <= ram[raddr];
    end

endmodule
